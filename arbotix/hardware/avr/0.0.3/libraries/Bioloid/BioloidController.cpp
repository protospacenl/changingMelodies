...
#define STEPSIZE 0.111328125f
#define STEPSIZE_RECIP  8.982456f
#define DEGR_PER_STEP   0.29f
...
void BioloidController::interpolateSpeedSetup(int time){
    int i;
    int frames = (time/BIOLOID_FRAME_LENGTH) + 1;
    ...
    for(i=0;i<poseSize;i++){
        ...
        if(nextpose_[i] > pose_[i]){
            diff = (nextpose_[i] - pose_[i]) >> BIOLOID_SHIFT;
        }else{
            diff = (pose_[i] - nextpose_[i]) >> BIOLOID_SHIFT;
        }

        //to RPM: ((Degr / Sec) * 60) / 360
        Serial.print(pose_[i]);
        Serial.print(" -> ");
        Serial.print(nextpose_[i]);
        Serial.print("("); Serial.print(diff); Serial.print("): ");
        rpm = ((diff * DEGR_PER_STEP) / ((float)time * 0.001 * 0.016)) * 0.0027;
        speed_[i] = frames + 1;
        speedParam = (int)(rpm * STEPSIZE_RECIP);

        Serial.print(rpm);
        Serial.print(", speed param: ");
        Serial.println(speedParam);

        if (speedParam < 0) speedParam = 0;
        if (speedParam > 1023) speedParam = 1023;

        dxlSetGoalSpeed(i+1, speedParam);
        pose_[i] = nextpose_[i];
        Serial.println(speedParam);
        delay(33);
    }
...
void BioloidController::interpolateSpeedStep(){
    if(interpolating == 0) return;
    int i;
    int complete = poseSize;
    while(millis() - lastframe_ < BIOLOID_FRAME_LENGTH);
